...
Our API currently utilizes API keys for authentication. To obtain your key, please <a href="#" onClick=function generateKey() { 
    ...
    if (email_address != '' && email_address != null) { 
        ...
        fetch('https://sandbox.finx.io/api/request-key/', {method: 'POST', body: JSON.stringify({email_address: email_address})}).then(
            response => response.json()).then(data => alert(
